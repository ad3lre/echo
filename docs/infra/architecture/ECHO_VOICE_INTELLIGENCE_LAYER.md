...
| ------------------------------- | ------------------------------------------ | ------------------------------------------------------------------------------------------------------------- |
| `echo_voice_participants` table | `backend/src/db/echoTables.ts`             | Tracks who is in which voice channel (server, channel, user, joined_at).                                      |
| `voice.ts` domain               | `backend/src/domain/echoStore/voice.ts`    | Join (permission + user-limit + ban checks), leave, list participants, moderation (disconnect / move).        |
| `echoVoice.ts` routes           | `backend/src/api/routes/echo/echoVoice.ts` | REST: `POST .../voice/join`, `POST .../voice/leave`, `GET .../voice/participants`, `POST .../voice/moderate`. |

### Frontend

| Component                | Location                                                                                                                     | What it does                                                                   |
| ------------------------ | ---------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------ |
| `voice.ts` API client    | `frontend/src/api/echo/voice.ts`                                                                                             | Fetch wrappers for join / leave / participants / moderate.                     |
| `voice.ts` orchestration | `frontend/src/services/orchestration/voice.ts`                                                                               | `createVoiceService` — calls REST join/leave, triggers workspace hydration.    |
| `voiceRouting.ts`        | `frontend/src/services/orchestration/voiceRouting.ts`                                                                        | Socket adapter emit stubs (`voice:join`, `voice:leave`).                       |
...
`shared/types/channel.ts` — `Channel.type: 'text' | 'voice'`, voice permission keys (`connect`, `speak`, `video`, `muteMembers`, `deafenMembers`, `moveMembers`, `useVoiceActivity`, `prioritySpeaker`, `stream`), `bitrateBps`, `userLimit`.
...
If LiveKit is replaced (e.g. custom Rust SFU per `[STACK.md](./STACK.md)`), only this adapter changes—**Echo product logic stays on `DesiredParticipantControl` + diff semantics**, not LiveKit types.
...
### 7.3 Alerting (examples for `monitoring/prometheus/rules/`)
...
Use `**livekit-client`** (official LiveKit JS SDK). Import it **only\*\* from a dedicated module (`frontend/src/services/voice/livekitAdapter.ts`) so the rest of the app never references LiveKit types directly.
...
1. **No LiveKit imports outside the adapter.** Backend: only `backend/src/voice/livekitAdapter.ts` imports `livekit-server-sdk`. Frontend: only `frontend/src/services/voice/livekitAdapter.ts` imports `livekit-client`. Enforce with an ESLint `no-restricted-imports` rule.
