...
1. **Settings → YouTube → Connect YouTube channel** (`POST /api/v1/auth/youtube/start` → Google consent).
2. In a **stage** channel, users with **Manage Channels** see **Go live on YouTube** (title + privacy).
3. Echo creates a YouTube `liveBroadcast` + `liveStream`, binds them, starts **LiveKit room composite RTMP egress** to the ingest URL, then transitions the broadcast to **live**.
4. **End live** stops egress and completes the YouTube broadcast.

## API (authenticated)

| Method | Path | Purpose |
| ------ | ---- | ------- |
| `GET` | `/api/v1/me/youtube` | Link status + channel profile |
| `DELETE` | `/api/v1/me/youtube` | Unlink channel |
