...
One forgotten string sort causes **subtle** bugs: wrong message order, broken pagination cursors, flaky “load older” edges. Add a **shared `compareEchoPublicId(a, b): number`** in `shared/` and use it everywhere.
...
1. Add **`shared/`** module: `nextSnowflakeId()`, `parseSnowflakeTime()`, `isValidEchoPublicId()` implementing the **§2.2 public string contract** (digits-only, length bounds, reject padding, **reject** prefixed / decorated forms).
...
- **Client gate**: `frontend/src/utils/echoIds.ts` and all `isEchoUuid` call sites (AppLayout, useEchoHistory, ServerSettingsModal, MessageList, useMockData, useDmSocialActions, …).
- **Shared contracts**: `shared/types`, `shared/echoContractV1.ts` event payloads (ids are opaque strings today — good).
...
- **Shared ordering helper**: `compareEchoPublicId` (or equivalent) in `shared/`; ban raw string sort for timeline order in lint/docs.
