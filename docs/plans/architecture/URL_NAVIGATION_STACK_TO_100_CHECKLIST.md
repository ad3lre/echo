...
**Non-goals:** UX redesign; refactors to [`useAppLayoutController.ts`](../../../frontend/src/features/layout/composables/useAppLayoutController.ts) (separate hotspot).

---

## Definition of done (100%)

- [x] Workspace-aware guild fallbacks and channel validation live in pure [`urlNavigationResolve.ts`](../../../frontend/src/features/layout/urlNavigationResolve.ts) (`pickFallbackParsedPath`, `resolveGuildPath`, `channelValidInServer`).
- [x] Pre-apply pathname normalization (guild adjust, `unknown` → fallback) in [`urlNavigationLocationNormalize.ts`](../../../frontend/src/features/layout/urlNavigationLocationNormalize.ts) (`normalizeBrowserPathParsed`).
- [x] Modal query open/strip + desired search patch in [`urlNavigationModalPolicy.ts`](../../../frontend/src/features/layout/urlNavigationModalPolicy.ts) (`planModalQueryEffectsFromUrl`, `buildModalSearchPatchFromState`).
- [x] [`useUrlNavigationSync.ts`](../../../frontend/src/features/layout/composables/useUrlNavigationSync.ts) = History API + watches + ref wiring; parse/format + surface→path remain [`urlNavigation.ts`](../../../frontend/src/features/layout/urlNavigation.ts); rail transitions remain [`navigationReducer.ts`](../../../frontend/src/features/layout/navigationReducer.ts).
