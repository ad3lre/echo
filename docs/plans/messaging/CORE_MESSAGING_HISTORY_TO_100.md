...
**Status (2026-04-11):** Pillar **1 — Core messaging & history** is at **100%** in [`STATUS_AND_PRODUCTION_READINESS.md`](../STATUS_AND_PRODUCTION_READINESS.md). **§6 — Shipped initiatives** is the authoritative “what exists today” summary. **§2 — Gap ledger** is a **frozen planning artifact** from before those ships; do not treat it as a current backlog.
...
| **Search (MVP+)**         | Server- and channel-scoped `GET …/messages/search` with `q`, `authorId`, `mentions`, `hasType`, **`hasAttachment`**; `pg_trgm` index on `content`; rate limits + metrics (see [`ECHO_CONTRACT_V1.md`](../contract/ECHO_CONTRACT_V1.md)).                       |
...
| **0.5** | **Pillar confidence tests**    | Pipeline covers many Echo paths; message-specific **integration** depth (reactions/pins/REST create) not fully mirrored in CI.                                                                                                | Extend [`echo.pipeline.integration.ts`](../../backend/src/tests/echo.pipeline.integration.ts) or add `echo.messaging.*` tests for new APIs; optional frontend contract test for reaction payload shape.                                               |
...
When **row 1** moves toward **100%**, update [`STATUS_AND_PRODUCTION_READINESS.md`](../STATUS_AND_PRODUCTION_READINESS.md):
...
| Tests              | [`echo.pipeline.integration.ts`](../../backend/src/tests/echo.pipeline.integration.ts) covers GET message, POST + idempotent replay, reaction PUT, pins GET/POST, read-state GET/PUT, `hasAttachment` search.           |
