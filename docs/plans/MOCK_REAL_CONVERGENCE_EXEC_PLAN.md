...
- `server/backend/src/config.ts` — canonical backend storage mode via `**ECHO_BACKEND_STORAGE=memory|postgres`**; `allowMockApi`; production requires `**ECHO_BACKEND_STORAGE=postgres**`+`**DATABASE_URL\*\*`.
- `server/backend/src/api/routes/mock.ts` + conditional registration in `routes/index.ts` — `/api/v1/mock/*` (dev only).
- `server/backend/src/db/mockdb.ts` — in-memory canonical mock.
- `server/backend/src/sockets/echoMessageFlow.ts` — `**echo_persisted` | `reject_unknown` only\*\* (legacy removed).

**Frontend**

- `clients/web/src/config.ts` — `USE_MOCK_DATA` / `IS_REAL_API_MODE` from `VITE_USE_MOCK_DATA`.
- `clients/web/src/echoMode.ts` — blocks Echo REST in mock mode.
- `clients/web/src/api/mock.ts` — client for `/api/v1/mock/*`.
- `clients/web/src/composables/useMockData.ts` — large workspace state + mock seeding; `AppLayout.vue` branches vs `hydrateEchoFromApi`.
...
- **Delete `contracts/` demo fixtures** immediately — they can remain as **data** for seeds; they must not select runtime mode.
...
5. **Optional:** CI job `rg` fails if `USE_MOCK_DB` appears outside an allowlist (`docs/`, `server/ops/scripts/seed`, `**/tests/`\*\*).
