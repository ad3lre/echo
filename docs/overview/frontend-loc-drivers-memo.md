...
|    3 | `views/FounderDashboardView.vue`                                         |  3250 | 1217 |    464 |  1566 | Mixed            | **Med–high** (lazy route / split founder-only island; large **style** block) | **Med** (product surface; design churn)                                                  |
|    4 | `composables/useLiveKitVoiceRoom.ts`                                     |  2959 |    0 |   2959 |     0 | Justified        |                               **Low** (refactor for clarity, not vanity LOC) | **High** (voice correctness)                                                             |
|    5 | `features/voice/components/VcActivityStage.vue`                          |  2941 |  938 |    958 |  1042 | Justified        |                                                                      **Low** | **High**                                                                                 |
|    6 | `components/chat/MessageList.vue`                                        |  2517 |  219 |   2290 |     5 | Mixed            |            **Med** (template small; script-heavy list logic—extract helpers) | **High** (hot path)                                                                      |
|    7 | `features/channel-panel/components/ChannelPanelList.vue`                 |  2394 |  933 |   1000 |   458 | Mixed            |                                        **Med** (presentational + list state) | **Med–high**                                                                             |
|    8 | `features/server-settings/components/ServerSettingsRolesSection.vue`     |  2340 | 1697 |    641 |     0 | Mixed            |                               **Med** (huge template—subcomponents / tables) | **Med** (RBAC UX)                                                                        |
|    9 | `features/layout/components/AppLayoutChatHeader.vue`                     |  2290 | 1266 |   1022 |     0 | Mixed            |                                                                      **Med** | **Med**                                                                                  |
|   10 | `features/layout/composables/useAppLayoutDmCalls.ts`                     |  2143 |    0 |   2143 |     0 | Justified        |                                                                  **Low–med** | **High**                                                                                 |
...
9. **`FounderDashboardView.vue`:** isolate to **lazy route** or separate workspace if product allows—reduces mainline cognitive load; total repo LOC may not drop unless feature shrinks.
10. **Voice megamodules (`useLiveKitVoiceRoom.ts`, `VcActivityStage.vue`):** default stance **justified**—only refactors driven by bugs, tests, or product scope change.
