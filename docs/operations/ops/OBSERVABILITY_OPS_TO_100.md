...
| Capability                          | Notes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| ----------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Prometheus exposition**           | **`GET /api/v1/metrics`** ([`health.ts`](../../../backend/src/api/routes/health.ts)) returns `prom-client` registry text; default Node metrics prefixed `echo_` via [`echoMetrics.ts`](../../../backend/src/observability/echoMetrics.ts).                                                                                                                                                                                                                                                                                   |
| **Domain metrics (Echo)**           | Socket **branch**, **`message_failed`**, **persist**, **handler duration**, **workspace** events / snapshot rejections, **permission** denials, **snowflake** wait + sequence, **message search** duration + result count; **REST RED** slice: `echo_rest_http_requests_total`, `echo_rest_http_request_duration_seconds` (label `route_group`); **`echo_dm_open_total{outcome}`**. Wired from handlers, routes, and [`echoHttpObservability.ts`](../../../backend/src/bootstrap/echoHttpObservability.ts).                  |
| **Alert / recording rules as code** | [`monitoring/prometheus/rules/`](../../../monitoring/prometheus/rules/) — failures, socket `reject_unknown` vs `echo_persisted`, snowflake wait rate, search p95. Scrape and layout: [`monitoring/README.md`](../../../monitoring/README.md).                                                                                                                                                                                                                                                                                |
| **Starter Grafana**                 | [`monitoring/grafana/echo-overview.json`](../../../monitoring/grafana/echo-overview.json) — import and point at your Prometheus datasource.                                                                                                                                                                                                                                                                                                                                                                                  |
| **Structured logs**                 | Fastify/pino with stable **`msg`** keys; **`registerEchoHttpObservability`** binds **`correlationId`** on `req.log` from Fastify **`X-Request-Id`** / `req.id`. [`gatherEchoPostMessageFailureDiagnostics`](../../../backend/src/domain/echoStore/access.ts) accepts optional **`correlationId`**; REST message post and socket handler pass it through. Sockets: optional **`X-Request-Id`** on handshake used when the message payload has no `correlationId` ([`handlers.ts`](../../../backend/src/sockets/handlers.ts)). |
| **Health endpoint**                 | **`GET /api/v1/health`** — DB, optional NATS, `useMockDb` ([`health.ts`](../../../backend/src/api/routes/health.ts)).                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| **Architecture alignment**          | [`STACK.md`](../../overview/STACK.md), [`ECHO_CONTRACT_V1.md`](../../contracts/ECHO_CONTRACT_V1.md) §Metrics (scrape path, series list, cardinality note, pointer to `monitoring/`).                                                                                                                                                                                                                                                                                                                                         |

**OpenTelemetry (optional):** The Node process loads **`@opentelemetry/sdk-node`** with auto-instrumentation when **`OTEL_EXPORTER_OTLP_ENDPOINT`** is set ([`otel.ts`](../../../backend/src/observability/otel.ts)); otherwise the SDK does not start (no overhead). This is **orthogonal** to **`X-Request-Id` / `correlationId`** logging (§7). **Frontend RUM** and **vendor-specific** Grafana Cloud / Datadog wiring stay outside this repo.
...
| **5**     | Alert rules + Grafana as code  | **Done:** [`monitoring/prometheus/rules/`](../../../monitoring/prometheus/rules/), [`echo-overview.json`](../../../monitoring/grafana/echo-overview.json), [`monitoring/README.md`](../../../monitoring/README.md).           |
| **4**     | Core REST RED metrics          | **Done:** `echo_rest_http_*` + `echo_dm_open_total` in [`echoMetrics.ts`](../../../backend/src/observability/echoMetrics.ts), hooks in [`echoHttpObservability.ts`](../../../backend/src/bootstrap/echoHttpObservability.ts). |
...
| Concern                       | Location                                                                                                    |
| ----------------------------- | ----------------------------------------------------------------------------------------------------------- |
| Metrics registry              | [`backend/src/observability/echoMetrics.ts`](../../../backend/src/observability/echoMetrics.ts)             |
| REST RED + correlation logger | [`backend/src/bootstrap/echoHttpObservability.ts`](../../../backend/src/bootstrap/echoHttpObservability.ts) |
| Metrics HTTP route            | [`backend/src/api/routes/health.ts`](../../../backend/src/api/routes/health.ts) (`/metrics`)                |
| Contract metrics list         | [`docs/contracts/ECHO_CONTRACT_V1.md`](../../contracts/ECHO_CONTRACT_V1.md) §Metrics                        |
| Prometheus rules              | [`monitoring/prometheus/rules/`](../../../monitoring/prometheus/rules/)                                     |
...
- `@opentelemetry/sdk-node` + `@opentelemetry/auto-instrumentations-node` integrated in `backend/src/observability/otel.ts`.
