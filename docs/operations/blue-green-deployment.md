...
| `npm run deploy`      | Build **idle** slot (`npm ci` + `npm run build`), stop any prior idle-slot API pid, run **`scripts/kill-dev-ports.mjs`** with `ECHO_FREE_PORTS=<idle>` (same safety as `prod:serve`), start API on idle port, **`GET /api/v1/health`**, rewrite proxy snippet, reload Caddy, stop previous slot’s API. |
