...
| 3    | When API stays down for **45 minutes** → force-restart: stop live `prod:serve`, free ports, start `prod:serve`, restart PM2 apps (`echo-backend`, `echo-web`, `echo-video-hls-worker`, `echo-marketing`, `echo-discord-bot`, `echo-watchdog`) |
...
Edit `WorkingDirectory` and `EnvironmentFile` paths in `server/ops/infra/systemd/echo-recovery-watcher.service` if your checkout is not `/opt/echo`.
...
node server/ops/scripts/echo-recovery-watcher.mjs --once
```
