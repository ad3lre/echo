...
8. **`/api/v1/health`** — Safe for probes; detailed fields (`backendStorageMode`, `useMockDb`) can be omitted in production with **`ECHO_HEALTH_REDACT=true`** (default in production). Set **`ECHO_HEALTH_REDACT=false`** only if operators need the extra fields.
