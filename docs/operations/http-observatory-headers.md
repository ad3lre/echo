...
The frontend build ships [`frontend/public/_headers`](../../frontend/public/_headers). It is copied into `frontend/dist/` and picked up by **Cloudflare Pages** and **Netlify** for matching static responses.
...
Add two static response headers (same values as in `frontend/public/_headers`):

- `Strict-Transport-Security`: `max-age=31536000; includeSubDomains; preload`
- `Content-Security-Policy`: copy the single-line policy from [`frontend/public/_headers`](../../frontend/public/_headers) (the long `Content-Security-Policy:` value after the header name).
