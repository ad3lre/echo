...
- Entitlements: [`src-tauri/Entitlements.macos.plist`](../src-tauri/Entitlements.macos.plist) (`webcredentials:chat-echo.com`)
- AASA `webcredentials.apps` must include `7HBQV8236H.com.echo.desktop` (generated by [`scripts/generate-apple-app-site-association.mjs`](../scripts/generate-apple-app-site-association.mjs); deploy with the frontend)
- App ID **`com.echo.desktop`** needs the **Associated Domains** capability in the Apple Developer portal
- Build must be **Developer ID signed** (ad-hoc `-` installs ignore associated-domain entitlements)

After changing entitlements or AASA, delete the app and reinstall. Login fields use standard `autocomplete` + `name` attributes for Password AutoFill.
