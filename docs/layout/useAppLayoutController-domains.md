...
Inventory of concern areas inside `[frontend/src/features/layout/composables/useAppLayoutController.ts](../../frontend/src/features/layout/composables/useAppLayoutController.ts)` (~3.06k current source lines; still a large orchestrator despite recent extraction). The composable is the **shell orchestrator**: it wires Pinia stores, extracted layout composables, socket/realtime, and the large `AppLayoutControllerContext` exposed to the layout.

**Split out of this file (controller still owns context assembly and registers late hooks):**
