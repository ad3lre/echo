...
- **[Apple native apps](./clients/apple/README.md)** — SwiftUI iOS/macOS clients; passkeys docs under `docs/operations/ios-passkeys.md`.
...
1. **CI precheck:** `npm run ci:precheck` from the repo root (or enable `./server/ops/scripts/setup-githooks.sh` so `git push` runs it automatically on non-GitHub remotes).
...
3. **Tests:** For tight scopes you may run a subset instead of full `ci:precheck`, for example:
   - `npm run test -w web -- --run <path-to-test>.ts`
...
- Follow existing patterns in the package you touch (`clients/web/`, `server/backend/`, etc.).
...
See **[docs/DEVELOPMENT.md](./docs/DEVELOPMENT.md)** (“Maintainer: publishing a fresh GitHub repo”). Summary: export a clean tree, run **gitleaks** and `node server/ops/scripts/check-oss-artifacts.mjs`, `git init` + single initial commit (or a tiny number of intentional commits), enable GitHub security reporting and branch protection.
...
A sample run at the **root** `dependencies` (not all workspaces) reported SPDX families including **MIT**, **Apache-2.0**, **BSD-3-Clause**, **0BSD**, and **(Apache-2.0 AND BSD-3-Clause)**. **Re-run before each release** and scan **each workspace** (`clients/web/`, `server/backend/`, `bot/`, …) because the root summary does not replace per-package compliance review.
