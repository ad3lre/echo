...
mkdir -p "$(dirname "$LOG")"
chmod +x "$ROOT/server/ops/scripts/cleanup-stale-ssh.sh"
CRON_LINE="${CRON_SCHEDULE} TZ=${TZ_NAME} bash ${ROOT}/server/ops/scripts/cleanup-stale-ssh.sh >>${LOG} 2>&1 ${MARKER}"
