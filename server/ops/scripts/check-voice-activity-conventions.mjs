 ...
 *                                  `isNewer*Tick` outside server/voice/contracts/.
 *   3. Per-game host election    — no copied `*OrchestratorUserId`/`*ArbiterUserId`
 *                                  declaration outside server/voice/contracts/.
...
for (const file of files) {
  ...
  if (r.endsWith('.vue')) {
    // path: clients/web/src/features/voice/<game>/.../components/<File>.vue
    const after = r.slice(`${cfg.voiceRoot}/`.length);
    const segs = after.split('/');
    const game = segs[0];
    const inComponents = segs.includes('components');
    // Only police nested game folders, not the shared server/voice/components dir.
