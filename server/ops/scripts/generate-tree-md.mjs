/**
 * Regenerates docs/overview/tree.md — run from repo root: node server/ops/scripts/generate-tree-md.mjs
 *
...
function shouldSkipRel(rel) {
  ...
  if (
    rel === 'clients/web/public/twemoji' ||
    rel.startsWith('clients/web/public/twemoji/')
  ) {
    return true;
  }
  // Any path segment named exactly `assets` (clients/web/src/assets, bot export bundles, …)
...
out.push('');
out.push('|-- server/backend/ (Node API + Echo domain)');
out.push('|   |-- src/');
...
out.push(
  '- **Omitted from the detailed tree (ignored assets):** any directory segment named `assets/` (e.g. `clients/web/src/assets/`, `bot/exports/.../assets/`) and `clients/web/public/twemoji/` — line totals exclude these paths.',
);
out.push(
  '- Line counts exclude comment-only lines for common source/config formats (`//`, `/* */`, `#`, `<!-- -->` by extension).',
);
out.push(
  '- Additional generated/runtime noise is omitted (`server/backend/.diagnostics/`, `server/backend/data/echo-local-uploads/`, `bot/exports/`, `logs/`), and files with 0 counted lines are hidden from the detailed tree.',
);
