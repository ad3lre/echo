...
#
# Usage:
#   sudo bash server/ops/scripts/setup-swap.sh
#   sudo ECHO_SWAP_SIZE_GB=2 bash server/ops/scripts/setup-swap.sh
#
# Env:
...
if [[ "$(id -u)" -ne 0 ]]; then
  echo "setup-swap: run as root (sudo bash server/ops/scripts/setup-swap.sh)" >&2
  exit 1
fi
