...
const roundedSvgPath = join(
  repoRoot,
  'clients/web/public/echo-rounded-trans.svg',
);
const echoMarkSvgPath = join(repoRoot, 'clients/web/src/assets/icons/echo.svg');
const outDir = join(repoRoot, 'clients/web/public/icons');
...
async function main() {
  ...
  await rasterizeBrandAsset(
    echoMarkSvgPath,
    join(repoRoot, 'clients/web/src/assets/icons/echo-logo.png'),
  );
  await rasterizeBrandAsset(
    echoMarkSvgPath,
    join(repoRoot, 'clients/web/public/echo-logo.png'),
  );
  await rasterizeBrandAsset(
    roundedSvgPath,
    join(repoRoot, 'clients/web/src/assets/icons/echo-rounded-logo.png'),
  );
  await rasterizeBrandAsset(
    roundedSvgPath,
    join(repoRoot, 'clients/web/public/echo-rounded-logo.png'),
  );
  await rasterizeBrandAsset(
    roundedSvgPath,
  ...
  const appIcon512Path = join(repoRoot, 'clients/web/public/echo-logo.png');
  ...
  await rasterizeRounded(32).toFile(
    join(repoRoot, 'clients/web/public/favicon.ico'),
  );
