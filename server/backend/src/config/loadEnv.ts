...
/**
 * Find monorepo root (directory containing `server/backend/package.json`) when running from
 * server/backend/src, server/backend/dist/..., or nested paths.
 */
function resolveRepoRootFromDir(startDir: string): string | undefined {
  let dir: string = startDir;
  for (let i = 0; i < 12; i++) {
    const backendPkg = path.join(dir, 'server', 'backend', 'package.json');
...
/**
 * Load env in order: repo root `.env` → `server/backend/.env` (fills keys not set by root) → `.env.lan` (overrides all).
 * Use `server/backend/.env` for vars you do not want in root `.env`; duplicate keys always follow root (unless `.env.lan` overrides).
 */
...
if (repoRoot && process.env.ECHO_CONFIG_TEST_ISOLATION !== '1') {
  ...
  const backendEnv = path.join(repoRoot, 'server', 'backend', '.env');
