...
import { migrateMembersRoleHierarchyPositions } from '../domain/echoStore/roles/roles';
...
async function migrateEchoCategorySchema(pool: pg.Pool): Promise<void> {
  ...
  await pool.query(`
    CREATE TABLE IF NOT EXISTS echo_vc_watch_together_sessions (
  ...
  `);
  await pool.query(`
    CREATE INDEX IF NOT EXISTS echo_vc_watch_together_sessions_host
    ON echo_vc_watch_together_sessions (host_user_id, updated_at DESC);
  `);
  ...
  await runEchoSchemaMigrationOnce(
    ...
    async () => {
      ...
      const { ensureGlobalRoleCategoryForServer } =
        await import('../domain/echoStore/roles/roleCategoryGlobals');
