...
function resolveDisposableDomainsFile(): string {
  // Source: server/backend/src/auth → ../../data
  // Compiled (rootDir=repo): server/backend/dist/server/backend/src/auth → ../../../../../data
  const candidates = [
    path.resolve(__dirname, '../../data/disposable-email-domains.txt'),
    path.resolve(__dirname, '../../../../../data/disposable-email-domains.txt'),
    path.resolve(
      process.cwd(),
      'server/backend/data/disposable-email-domains.txt',
    ),
    path.resolve(process.cwd(), 'data/disposable-email-domains.txt'),
  ];
  for (const p of candidates) {
    if (fs.existsSync(p)) return p;
  }
  throw new Error(
    `[echo] Missing server/backend/data/disposable-email-domains.txt (searched from ${__dirname})`,
  );
}
