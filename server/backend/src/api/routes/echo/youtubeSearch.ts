...
import { tryConsumeYoutubeWatchTogetherSeconds } from '../../../services/watchTogether/youtubeWatchTogetherUsageBudget';
import { YOUTUBE_SEARCH_FETCH_MS } from '../../../constants/outboundHttp';
import { createUpstreamSearchCache } from '../../../services/search/upstreamSearchCache';
...
async function consumeYoutubeBrowseBudget(
  ...
  const cost = config.youtubeWatchTogetherBrowseCostSec;
  if (!cost || cost <= 0) return true;
  const r = await tryConsumeYoutubeWatchTogetherSeconds(userId, cost);
  if (r.ok) return true;
  const msg =
    r.reason === 'global'
