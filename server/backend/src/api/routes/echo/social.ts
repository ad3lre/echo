...
export default async function echoSocialRoutes(
  ...
  fastify.get<{ Querystring: { ids?: string } }>(
    ...
    async (req, reply) => {
      ...
      if (userIds.length > ECHO_PROFILE_BATCH_MAX_USER_IDS) {
        return sendError(
          reply,
          400,
          'INVALID_QUERY',
          `At most ${ECHO_PROFILE_BATCH_MAX_USER_IDS} user ids per request`,
        );
      }
      const visibleUserIds = await filterProfileVisibleEchoUserIds(
        pool,
        getAuthUser(req).id,
        userIds,
      );
      const profiles = await getEchoUserPublicProfileRows(pool, visibleUserIds);
      return reply.code(200).send({ profiles });
    },
  );
