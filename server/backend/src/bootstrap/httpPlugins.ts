...
/** Browser preflight must allow every non-simple header Echo clients send (see `clients/web/src/api/echo/transport.ts`). */
...
export async function registerHttpPlugins(
  ...
  await fastify.register(cors, {
    ...
    /** Engine.IO / some proxies omit strict preflight headers; default `true` can yield 400 on OPTIONS. */
