...
async function run(): Promise<void> {
  ...
  try {
    ...
    const { runDiscordImportStep } =
      await import('../../services/discordImport/discordImport');
    const { mergeDiscordShadows } =
      await import('../../domain/discord/discordShadowMerge');
    const { upsertDiscordUserLink } =
      await import('../../domain/discord/discordUserLinkRepo');
