...
function clearConfigAndRoutes() {
  ...
  clearRequireCacheBySubstring([
    ...
    `${path.sep}backend${path.sep}src${path.sep}services${path.sep}uploads${path.sep}storedMediaUrl.`,
    `${path.sep}backend${path.sep}src${path.sep}services${path.sep}uploads${path.sep}s3UploadPresign.`,
    `${path.sep}backend${path.sep}src${path.sep}services${path.sep}discordImport${path.sep}discordImportAuthorization.`,
    ...
    `${path.sep}backend${path.sep}src${path.sep}api${path.sep}routes${path.sep}echo${path.sep}invites.`,
  ]);
}
...
async function run(): Promise<void> {
  ...
  try {
    ...
    const { default: echoInvitesRoutes } =
      await import('../../api/routes/echo/invites');
...

  {
    ...
    try {
      ...
      const {
        ...
        await import('../../services/discordImport/discordImportAuthorization');
      const managedGuild = {
...

  {
    ...
    try {
      ...
      const { validateEchoStoredBrandingUrl } =
        await import('../../services/uploads/storedMediaUrl');
...

  {
    ...
    try {
      ...
      const { buildEchoUploadPublicUrlForStorageKey } =
        await import('../../services/uploads/s3UploadPresign');
      assert.equal(
