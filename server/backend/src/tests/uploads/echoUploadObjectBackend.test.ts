 ...
 * Run: node --import tsx server/backend/src/tests/uploads/echoUploadObjectBackend.test.ts
 */
...
function clearBackendConfigCache() {
  ...
  for (const k of Object.keys(require.cache)) {
    if (
      ...
      k.includes(
        `${path.sep}backend${path.sep}src${path.sep}services${path.sep}uploads${path.sep}echoUploadObjectBackend.`,
      )
    ) {
      // eslint-disable-next-line @typescript-eslint/no-dynamic-delete
...
async function loadUploadRouting() {
  ...
  // eslint-disable-next-line @typescript-eslint/no-var-requires
  const backend =
    require('../../services/uploads/echoUploadObjectBackend') as typeof import('../../../services/uploads/echoUploadObjectBackend');
  // eslint-disable-next-line @typescript-eslint/no-var-requires
  const presign =
    require('../../services/uploads/s3UploadPresign') as typeof import('../../../services/uploads/s3UploadPresign');
  return {
    ...backend,
