 ...
 * Run: node --import tsx server/backend/src/tests/uploads/echoUploadStorageKeyPath.test.ts
 */
...
function clearConfigCache(): void {
  for (const k of Object.keys(require.cache)) {
    ...
    if (
      k.includes(
        `${path.sep}backend${path.sep}src${path.sep}services${path.sep}uploads${path.sep}localUploadDisk.`,
      )
    ) {
      // eslint-disable-next-line @typescript-eslint/no-dynamic-delete
      delete require.cache[k];
...
async function runLocalResolveTests(): Promise<void> {
  ...
  try {
    const { resolveLocalUploadFilePath } =
      await import('../../services/uploads/localUploadDisk');
