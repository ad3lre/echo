...
struct EchoProfileEditView: View {
  ...
  @State private var presenceStatus = "offline"
  @State private var originalPresenceStatus = "offline"
  ...
  var body: some View {
    ScrollView(showsIndicators: false) {
      VStack(spacing: 0) {
        ...
        ZStack(alignment: .bottom) {
          ...
          .overlay(alignment: .topTrailing) {
            PhotosPicker(selection: $bannerItem, matching: .images) {
              ProfileMediaEditButton(accessibilityLabel: "Change profile banner")
            }
            .buttonStyle(.plain)
            .padding(16)
          }
          ...
          HStack(alignment: .bottom, spacing: 8) {
            EchoEditableAvatar(
              ...
              PhotosPicker(selection: $avatarItem, matching: .images) {
                ProfileMediaEditButton(accessibilityLabel: "Change profile picture")
              }
              .buttonStyle(.plain)
            }
        ...
        VStack(alignment: .leading, spacing: 10) {
          ...
          VStack(spacing: 0) {
            ProfileInlineField(title: "Display name") {
              TextField("Display name", text: $displayName)
            }
            ProfileInlineField(title: "Bio") {
              TextField("Add a bio", text: $bio, axis: .vertical)
                .lineLimit(2...5)
            }
            ProfileInlineField(title: "Status") {
              TextField("Set a status", text: $status)
            }
            ProfilePresencePicker(selection: $presenceStatus)
          }
          .padding(6)
          .background(
            Color(red: 0.024, green: 0.027, blue: 0.038).opacity(0.98),
    ...
    .task {
      ...
      if let loadedPresence = try? await client.loadPresence(
        userID: userID, accessToken: accessToken)
      {
        presenceStatus = loadedPresence
        originalPresenceStatus = loadedPresence
        presenceLoaded = true
      }
    }
  ...
  private func save() {
    ...
    Task {
      do {
        ...
        if presenceLoaded, presenceStatus != originalPresenceStatus {
  ...
  private var hasChanges: Bool {
    displayName != (profile?.name ?? "")
      ...
      || (presenceLoaded && presenceStatus != originalPresenceStatus)
      || bannerData != nil
      || avatarData != nil
  }
  ...
  var body: some View {
    HStack(spacing: 10) {
      VStack(alignment: .leading, spacing: 2) {
        ...
        Text("Save to update your profile")
      ...
      Button(action: onSave) {
        ...
        .frame(height: 36)
        .background(
          Color(red: 0.38, green: 0.30, blue: 0.86), in: RoundedRectangle(cornerRadius: 11))
      }
      .buttonStyle(.plain)
      .disabled(isSaving)
    }
    ...
    .padding(.vertical, 8)
    .background(
      Color(red: 0.055, green: 0.058, blue: 0.075).opacity(0.98),
...
private struct ProfilePresencePicker: View {
  ...
  private let options = [
    (id: "online", label: "Online", color: Color(red: 0.20, green: 0.78, blue: 0.46)),
    (id: "idle", label: "Idle", color: Color(red: 0.96, green: 0.68, blue: 0.22)),
    (
      id: "do_not_disturb", label: "Do Not Disturb",
      color: Color(red: 0.94, green: 0.30, blue: 0.38)
    ),
  ]
...
private struct ProfileMediaEditButton: View {
...
private struct EchoEditableAvatar<Overlay: View>: View {
  let profile: EchoUserProfile?
  let baseURL: URL
