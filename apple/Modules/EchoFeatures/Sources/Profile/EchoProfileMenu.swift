...
struct EchoProfileMenu: View {
  ...
  var body: some View {
    VStack(spacing: 10) {
      EchoMenuAction(
        title: "Edit profile", icon: "person.crop.circle", tint: .indigo, action: onEditProfile)
      EchoMenuAction(title: "Settings", icon: "gearshape", tint: .blue, action: onSettings)
      EchoMenuAction(
        title: "Log out", icon: "rectangle.portrait.and.arrow.right", tint: .red, action: onSignOut)
    }
