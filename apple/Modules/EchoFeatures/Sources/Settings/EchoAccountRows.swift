      ...
      HStack(spacing: 12) {
        ...
        if let statusIcon {
          Image(systemName: statusIcon)
            ...
            .accessibilityLabel(statusIcon == "checkmark.seal.fill" ? "Verified" : "Not verified")
        }
...
struct EchoSessionRow: View {
  ...
  private var deviceName: String {
    ...
    return userAgent.isEmpty ? "Echo device" : userAgent
  }

  private var detail: String {
    var parts = [session.location ?? "Unknown location"]
      ...
      Button(role: .destructive, action: onRevoke) {
        Label(
          session.sessionIDs.count > 1
            ? "Revoke all \(session.sessionIDs.count) sessions" : "Revoke session",
...
struct EchoPasskeyRow: View {
  ...
  var body: some View {
    VStack(alignment: .leading, spacing: 11) {
      ...
      HStack(spacing: 10) {
        EchoCompactAction(title: "Rename", icon: "pencil", tint: .purple, action: onRename)
        EchoCompactAction(title: "Remove", icon: "trash", tint: .red, action: onRemove)
      }
    }
  }
}
