...
/// A session item is protected by the device owner's biometrics or passcode.
/// Passing the already-evaluated `LAContext` keeps the system prompt to one
/// native interaction when the user chooses Face ID or Touch ID.
public final class KeychainSessionStore: SessionStore, @unchecked Sendable {
  ...
  public func loadSession(authenticationContext: LAContext? = nil) throws -> EchoSession? {
    ...
    context.localizedReason = "Unlock your Echo session"
    var query = baseQuery
  ...
  public func saveSession(_ session: EchoSession) throws {
    ...
    guard
      let accessControl = SecAccessControlCreateWithFlags(
        ...
        return
      #else
        throw error?.takeRetainedValue() as Error? ?? SessionStoreError.invalidSessionData
      #endif
    }
