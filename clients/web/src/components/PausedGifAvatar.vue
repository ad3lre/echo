...
const props = withDefaults(
  defineProps<{
    src: string;
    alt: string;
    /** How the image fills the wrapper; `contain` fits whole asset (e.g. role emoji in a square tile). */
...
const mergedImgClass = computed(() => {
  const fit =
    props.imgObjectFit === 'contain' ? 'object-contain' : 'object-cover';
  return ['absolute inset-0 h-full w-full', fit, props.imgClass?.trim() || '']
    .filter(Boolean)
    .join(' ');
});
