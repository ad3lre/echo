...
} from '@/features/layout/composables/server/echoInviteShareUrl';
import {
  fetchEchoServerInviteLink,
  postEchoServerInvite,
} from '@/api/echoClient';
import { postEchoServerInviteCode } from '@/api/echo/serverApplications';
...
function formatDirectInviteError(error: unknown): string {
  if (error instanceof EchoApiError) {
    const m = error.message.trim();
    if (m) return m;
    if (error.status === 403)
...
export function useSelectedServerInvite(deps: {
  ...
  const directHexInviteLink = ref('');
  const directHexInviteBusy = ref(false);

  watch(isInviteModalOpen, (open) => {
    if (!open) {
      directHexInviteLink.value = '';
      directHexInviteBusy.value = false;
    }
  });
  ...
  async function createDirectHexInvite(): Promise<void> {
    const s = selectedServer.value;
    const sid = s?.id?.trim();
    if (!sid || sid === 'echo') return;
    const token = authSession.accessToken?.trim() ?? '';
    if (!authSession.isAuthenticated || !token) {
      dispatchAppToast('Sign in to create an invite link.', 'warning');
      return;
    }
    directHexInviteBusy.value = true;
    try {
      ...
      dispatchAppToast(formatDirectInviteError(e), 'warning');
    } finally {
      directHexInviteBusy.value = false;
    }
  }
