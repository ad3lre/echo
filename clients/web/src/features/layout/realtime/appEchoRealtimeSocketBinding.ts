...
import type { UseSocketBinding } from '@/features/layout/useSocket';
import type { EchoRealtimeHostPorts } from '@/features/layout/realtime/echoRealtimePort';
import { createEchoSocketRealtimeWiring } from '@/features/layout/realtime/echoSocketRealtimeWiring';
...
import { registerEchoRealtimeUiControllerTransactions } from '@/features/layout/realtime/echoRealtimeUiControllerTransactions';
import { createEchoSocketSendMessage } from '@/features/chat/send/echoSocketSendMessage';
import { createEchoSocketSubmitEmitters } from '@/features/layout/realtime/echoSocketSubmitEmits';
import {
  createPresenceHeartbeatSession,
  resolveRelayablePresenceForOutbound,
  tryEmitPresenceSet,
  type PresenceEmitAdapter,
} from '@/features/layout/realtime/socketPresenceSession';
import { applyEchoSocketActiveChannelChange } from '@/features/layout/realtime/echoSocketSessionLifecycle';
import type { EchoSocketAuthKey } from '@/features/layout/realtime/echoSocketSessionLifecycle';
import { registerChannelTypingSocketEmit } from '@/features/chat/channelTyping';
import { setEchoGameSocketConnected } from '@/features/games/echoGameSocketRegistry';
import { createEchoSocketInboundListeners } from '@/features/layout/realtime/echoSocketInboundListeners';
import { newCorrelationId } from '@/features/layout/realtime/socketOutbound';
import {
  createEchoRealtimeChatIngestPort,
  createEchoRealtimeTypingIngestPort,
} from '@/features/chat/ingest/echoRealtimeChatIngest';
import {
  clearSocketXhrPollReloadGuard,
  defaultSocketReloadGuardBrowser,
  tryReloadForSocketXhrPollError,
} from '@/features/layout/realtime/socketReloadGuard';
import { defaultEchoRealtimeBrowserEvents } from '@/features/layout/realtime/echoRealtimeBrowserEvents';
import { registerDeferredMediaOutboundSend } from '@/features/chat/send/deferredMediaOutboundSend';
import { createEchoRealtimePlatformSessionSync } from '@/features/layout/realtime/echoRealtimePlatformSessionSync';
import { ingestEchoSocketConnectError } from '@/features/layout/realtime/socketConnectErrorIngest';
  ...
  return {
    ...
    wiring: createEchoSocketRealtimeWiring({
      ...
      createDomain: (transport) => {
        ...
        return {
          port: port as unknown as import('@/features/layout/realtime/echoRealtimePort').EchoRealtimePort,
          inboundListeners,
          onBeforeTeardown: () => {
