...
vi.mock(
  '@/features/layout/realtime/socketTransport',
  () => socketTransportMock,
);
...
describe('createEchoSocketRealtimeWiring', () => {
  ...
  it('returns io, lifecycle handles, and the domain-provided port', () => {
    ...
    const port = {
      sendMessage: vi.fn(),
    } as unknown as import('@/features/layout/realtime/echoRealtimePort').EchoRealtimePort;
    const inboundListeners =
      {} as unknown as import('@/features/layout/realtime/socketInbound').EchoSocketInboundListeners;
  ...
  it('connectSocket resolves immediately when socket transport is disabled', async () => {
    const w = createEchoSocketRealtimeWiring({
      ...
      createDomain: () =>
        ({
          port: {} as unknown as import('@/features/layout/realtime/echoRealtimePort').EchoRealtimePort,
          inboundListeners:
            {} as unknown as import('@/features/layout/realtime/socketInbound').EchoSocketInboundListeners,
        }) as const,
    });
