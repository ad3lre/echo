...
/** GitHub Octicon paths (viewBox 0 0 16 16). */
export const MARKDOWN_ALERT_ICON_PATHS: Record<MarkdownAlertKind, string> = {
...
export function markdownAlertTitleLabel(kind: MarkdownAlertKind): string {
  return `${kind.slice(0, 1).toUpperCase()}${kind.slice(1)}`;
}

export function appendMarkdownAlertIcon(
  doc: Document,
  heading: HTMLElement,
  kind: MarkdownAlertKind,
): void {
  const svg = doc.createElementNS('http://www.w3.org/2000/svg', 'svg');
  svg.setAttribute('class', 'md-alert__icon');
  svg.setAttribute('viewBox', '0 0 16 16');
  svg.setAttribute('width', '16');
  svg.setAttribute('height', '16');
  svg.setAttribute('aria-hidden', 'true');
  const path = doc.createElementNS('http://www.w3.org/2000/svg', 'path');
  path.setAttribute('d', MARKDOWN_ALERT_ICON_PATHS[kind]);
