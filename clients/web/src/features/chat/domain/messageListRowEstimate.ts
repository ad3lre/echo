...
/** Compact OG card (provider + title + short description). */
const MESSAGE_LIST_EMBED_COMPACT_PX = 128;
...
function estimateLinkEmbedBlockPx(embed: Embed): number {
  ...
  let compact = MESSAGE_LIST_EMBED_COMPACT_PX;
  const desc = embed.description?.trim();
  if (desc) {
    compact += Math.min(72, Math.ceil(desc.length / 90) * 16);
  }
  const fieldCount = embed.fields?.length ?? 0;
  if (fieldCount > 0) {
    compact += Math.min(100, fieldCount * 28);
  }
  return compact;
}
