...
describe('MessageList runtime row synchronization', () => {
  ...
  it('clips virtual rows to slot height and measures an inner content node', async () => {
    // Regression: adeliverse-style message collision — absolute rows must set
    // height=slot size so overflow:hidden clips until measure catches up.
    const ids = ['m1', 'm2', 'm3'];
    const mapEntries = ids.map(
      (id) => [id, makeMessageWithAuthor(id)] as const,
    );
    ...
    const Wrapper = defineComponent({
      name: 'MessageListClipShellHarness',
      setup() {
        return () =>
          h(MessageList, {
            channelId: 'ch-clip-shell',
            messages: messages.value,
            initialHistoryLoading: false,
          });
      },
    ...
    const shells = [
      ...container.querySelectorAll('[data-cy="message-list-virtual-row"]'),
    ] as HTMLElement[];
    expect(shells.length).toBe(3);
    for (const shell of shells) {
      expect(shell.style.height).toBe('88px');
      expect(shell.style.transform).toMatch(/translate3d/);
    }
    ...
    expect(measures[0]?.getAttribute('data-index')).toBe('0');
    expect(measures[1]?.getAttribute('data-index')).toBe('1');
    expect(measures[2]?.getAttribute('data-index')).toBe('2');
    expect(container.querySelector('#message-m1')).not.toBeNull();
  });
});
