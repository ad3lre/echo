...
import PendingMediaPreview from '@/features/chat/components/PendingMediaPreview.vue';
import ImageViewerModal from '@/features/chat/components/ImageViewerModal.vue';
import type { ImageItem } from '@/features/chat/components/ImageViewerModal.vue';
import PollCreateModal from '@/features/chat/components/PollCreateModal.vue';
...
import { useEmojiAutocomplete } from '@/features/chat/emoji/useEmojiAutocomplete';
import { useSlashCommandAutocomplete } from '@/features/chat/composables/useSlashCommandAutocomplete';
import {
  useMentionAutocomplete,
  type MentionOption,
} from '@/features/chat/composables/useMentionAutocomplete';
import { useChannelAutocomplete } from '@/features/chat/composables/useChannelAutocomplete';
import { usePendingMedia } from '@/features/chat/composables/usePendingMedia';
import { usePendingVideoEagerUpload } from '@/features/chat/composables/usePendingVideoEagerUpload';
import { useChatSend } from '@/features/chat/composables/useChatSend';
import { usePopoutStack } from '@/features/chat/composables/usePopoutStack';
...
import { extractMediaFilesFromClipboard } from '@/features/chat/useClipboardMedia';
...
import { isEmojiOnlyUpTo12 } from '@/features/chat/emoji/emojiUtils';
import type {
  InsertUserMentionFn,
...
import type { ComposerSnapshot } from '@/features/chat/composables/useComposerState';
import { emitDiagnostic } from '@/observability/sessionDiagnostics';

/**
 * On-demand pickers — only mounted behind their `v-if="activePopout === …"`. Lazy
 * so the heavy emoji/gif/attach UI (and the GIF provider clients) load on first
 * open instead of bloating the chat-critical ChatInput chunk.
 */
const EmojiPopout = defineAsyncComponent(
  () => import('@/features/chat/components/EmojiPopout.vue'),
);
const GifPopout = defineAsyncComponent(
  () => import('@/features/chat/components/GifPopout.vue'),
);
const AttachPopout = defineAsyncComponent(
  () => import('@/features/chat/components/AttachPopout.vue'),
);
...
const slashCommandOptions = computed(() =>
  buildChatSlashCommands({
    mediaAllowed: attachAllowsUpload.value,
    pollAllowed: attachAllowsPoll.value,
    editingMessage: !!props.editingMessage,
  }),
);

function handleSlashCommandSelected(command: ChatSlashCommand) {
  switch (command.action.type) {
    ...
    case 'image-slot':
      if (composerBarDisabled.value) return;
      composer.insertImageSlot(command.action.aspectW, command.action.aspectH);
      composer.focus();
      break;
    case 'button-row':
      if (composerBarDisabled.value) return;
      composer.insertButtonRow([...command.action.buttons]);
      composer.focus();
      break;
    case 'markdown-wrap': {
      if (composerBarDisabled.value) return;
      const { prefix, suffix } = markdownWrapDelimiters(command.action.kind);
...
const slashCommandAutocomplete = useSlashCommandAutocomplete(
  ...
  {
    getCommands: () => slashCommandOptions.value,
  },
);

const showSlashCommandAutocomplete = computed(
  () => chatInputFocused.value && slashCommandAutocomplete.showPopup.value,
);
...
function refreshAutocomplete() {
  nextTick(() => {
    ...
    const mayTrigger =
      ...
      slashCommandAutocomplete.showPopup.value;
    if (!mayTrigger) {
      ...
      if (slashCommandAutocomplete.showPopup.value)
        slashCommandAutocomplete.close();
      return;
    }
    emojiAutocomplete.updateFromInput();
    mentionAutocomplete.updateFromInput();
    channelAutocomplete.updateFromInput();
    slashCommandAutocomplete.updateFromInput();
  });
}
...
function handleKeydown(e: KeyboardEvent): boolean {
  ...
  if (slashCommandAutocomplete.handleKeydown(e)) {
    return true;
  }
...
function handleSlashCommandAutocompleteSelect(command: ChatSlashCommand) {
  slashCommandAutocomplete.select(command);
  nextTick(() => composer.focus());
}
...
<template>
  ...
  >
    ...
    <ChatInputComposerBar
      ...
      :handle-slash-command-autocomplete-select="
        handleSlashCommandAutocompleteSelect
      "
