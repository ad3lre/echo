...
import ProfileBannerMedia from './ProfileBannerMedia.vue';
import { icons } from '@/assets/icons';
import { profileBannerRefractionBackdropStyle } from '@/features/member-profile/profileBannerGradientFromImage';
import UserProfileMoreMenu from './UserProfileMoreMenu.vue';
import ProfileUserBadges from './ProfileUserBadges.vue';
import ProfileFriendHeartBadge from './ProfileFriendHeartBadge.vue';
import ProfileMemberSinceInline from './ProfileMemberSinceInline.vue';
import ProfileVoiceActivityWidget from './ProfileVoiceActivityWidget.vue';
import ProfileVcActivityWidget from './ProfileVcActivityWidget.vue';
import { useProfileOngoingVcActivity } from '@/features/member-profile/useProfileOngoingVcActivity';
import type { ProfileOngoingVcActivity } from '@/features/member-profile/useProfileOngoingVcActivity';
import ProfileBioText from './ProfileBioText.vue';
import ProfileCustomStatusThoughtBubble from './ProfileCustomStatusThoughtBubble.vue';
import { useUserVoiceChannelPresenceForProfile } from '@/features/member-profile/useUserVoiceChannelPresenceForProfile';
import {
  isGuildMemberProfileContext,
  roleHierarchyDisplayRank,
  type ExpandedProfile,
} from '@/features/member-profile/memberProfiles';
import { memberPopoutRoleChipStyle } from '@/features/member-profile/memberPopoutRoleChipStyle';
import type { UserVoiceChannelPresence } from '@/features/member-profile/userVoiceChannelPresence';
import { selectPresence } from '@/features/layout/presence';
import { selectFriendshipUiState } from '@/features/layout/friendshipUi';
import {
...
const props = withDefaults(
  defineProps<{
    ...
    isFriend?: boolean;
    note: string;
    /** When set, hides add/remove friend for your own profile. */
    currentUserId?: string;
    /** You have blocked this user (Echo or mock). */
    isTargetBlocked?: boolean;
    /** Viewer is a guest (Echo): cannot use Friends; hides add/remove friend. */
    guestFriendsLocked?: boolean;
    /** Pending outgoing friend request to this profile (Echo + mock). */
    hasOutgoingFriendRequest?: boolean;
    /** Pending incoming friend request FROM this profile (Echo + mock). */
    ...
    /** Full social context (single authority) */
    friendIds?: string[];
    ...
    /**
     * When true, omit the Message control — e.g. user is already in a 1:1 DM with this peer.
     */
...
const emit = defineEmits<{
  ...
  /** Open Settings → Profile (viewer’s own expanded profile). */
  'open-edit-profile': [];
}>();
...
/** Hide add/remove friend when the open profile is the signed-in user (requires currentUserId). */
function expandedPresenceForPeer(
  ...
  return selectPresence({
    ...
    rowStatus,
    diagnosticsKey: `expanded-modal-peer:${userId ?? '?'}`,
...
const expandedMainPresence = computed(() => {
  ...
  if (!p) {
    return selectPresence({
      rowStatus: undefined,
      diagnosticsKey: 'expanded-modal:none',
    });
  }
...
/** ⋯ menu: show for any other user even when friend/Message actions are gated off. */
...
const isViewingSelfProfile = computed(() => {
  const p = props.profile;
...
/** Message / Friend / Edit / ⋮ — primary actions beside the display name. */
const showExpandedProfileHeadActions = computed(() => {
  if (!props.profile) return false;
  return (
    showFriendAction.value ||
    isViewingSelfProfile.value ||
    showProfileMoreMenuTrigger.value
  );
});

const showExpandedProfileMessageIcon = computed(() => {
  const p = props.profile;
  if (!p || props.isTargetBlocked) return false;
  if (!showFriendAction.value) return false;
  if (props.hideOpenDmButton) return false;
  return true;
});

const showMessageInEllipsisMenu = computed(() => {
  const p = props.profile;
  if (!p) return false;
  if (!showFriendAction.value) return false;
  if (props.hideOpenDmButton) return false;
  return !props.isTargetBlocked;
});
...
const isExpandedGuildContext = computed(() =>
  isGuildMemberProfileContext(props.profile?.serverName),
);

const sortedExpandedRoles = computed(() => {
  const p = props.profile;
  if (!p?.roles?.length) return [];
  return [...p.roles].sort(
    (a, b) =>
      roleHierarchyDisplayRank(b.name) - roleHierarchyDisplayRank(a.name),
  );
});
...
<template>
  <!-- Centered modal (servers/members) -->
  <Teleport to="body">
    <div
      v-if="modelValue && profile"
      class="ep-overlay fixed inset-0 z-[200] flex items-center justify-center"
      role="dialog"
      aria-modal="true"
      aria-labelledby="ep-title"
      @click="onOverlayClick"
    >
      <article class="ep-modal" @click.stop>
        <div class="ep-layout">
          ...
          <div class="ep-left">
            <div class="ep-banner-stack">
              ...
              <div class="ep-banner">
                <ProfileBannerMedia
                  ...
                  :session-key="`${profile.id}-ep-banner`"
            ...
            <div class="ep-top-band">
              <div class="ep-top-band__avatar ep-avatar-wrap--static-pfp">
                <PausedGifAvatar
              ...
              <div
                ...
                class="ep-top-band__custom-status"
              >
                <ProfileCustomStatusThoughtBubble
                  :text="profile.customStatus"
                />
              </div>
            </div>

            <!-- Main column: refraction sits outside the scroll box so filter:blur is not clipped by overflow-y:auto -->
            <div class="ep-left-main">
              ...
              <div class="ep-left-body custom-scrollbar">
                <div class="ep-name-block">
                  <div class="ep-profile-head-row">
                    <div class="ep-profile-head-row__primary">
                      <div
                        class="ep-name-heading flex min-w-0 items-center gap-2"
                      >
                        <h2
                          id="ep-title"
                          class="ep-name min-w-0 flex-1 truncate"
                        >
                          {{ profile.displayName }}
                        </h2>
                        <div
                        ...
                        >
                          ...
                          <div
                            ...
                            class="flex shrink-0 items-center gap-1 rounded bg-indigo-500/15 px-1.5 py-0.5 text-[8px] font-bold uppercase tracking-wider text-indigo-400 ring-1 ring-indigo-500/25"
                          >
                            <img
                              :src="icons.explore"
                              alt=""
                    ...
                    <div
                      v-if="showExpandedProfileHeadActions"
                      class="ep-profile-head-row__actions relative z-[4] min-w-0"
                    >
                      <button
                        v-if="isViewingSelfProfile"
                        type="button"
                        class="ep-header-btn ep-header-btn-message"
                        @click="emit('open-edit-profile')"
                      >
                        <span>Edit profile</span>
                      </button>
                      <button
                        v-if="showExpandedProfileMessageIcon"
                        type="button"
                        class="ep-header-btn ep-header-btn-icon-only"
                        title="Message"
                        aria-label="Message"
                      ...
                      <UserProfileMoreMenu
                        ...
                        :show-message-menu-item="showMessageInEllipsisMenu"
                        :show-quick-mention-in-menu="false"
                        :message-menu-item-disabled="!!isTargetBlocked"
                ...
                <section
                  v-if="
                    isExpandedGuildContext && sortedExpandedRoles.length > 0
                  "
                  class="ep-roles-block mt-4"
                >
                  <div class="ep-about-label">Roles in this server</div>
                  <div class="mt-2 flex flex-wrap gap-1.5">
                    <span
                      v-for="role in sortedExpandedRoles"
                      :key="role.id"
                      class="inline-flex max-w-full items-center gap-1 truncate rounded-full px-2.5 py-1 text-[11px] font-semibold text-foreground"
                      :style="memberPopoutRoleChipStyle(role.color)"
                    >
                      {{ role.name === '@everyone' ? 'Member' : role.name }}
                    </span>
                  </div>
                </section>

                <!-- About -->
                <div class="ep-about-block">
                  <div
                    v-if="
                      profile.bio?.trim() ||
                  ...
                  <div v-if="!profile.isDiscordShadow">
                    ...
                    <textarea
                      ...
                      @blur="commitNote"
                      @input="
                        resizeNoteInput($event.target as HTMLTextAreaElement)
                      "
                    />
                  </div>
                </div>
          ...
          <div class="ep-right">
            <div class="ep-right-body">
              ...
              <div
                class="ep-tab-content ep-tab-content--desktop custom-scrollbar"
              >
                <!-- Mutual Servers -->
