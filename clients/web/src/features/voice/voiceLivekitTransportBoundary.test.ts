 ...
 * `features/voice/voiceService.ts` — layout must not fork transport.
 */
const __filename = fileURLToPath(import.meta.url);
const __dirname = dirname(__filename);
/** `clients/web/src` — scan all TS/Vue; allow only voice orchestration + API + its unit test. */
const FRONTEND_SRC = join(__dirname, '..', '..', '..');

const ALLOW_REL_PREFIXES = [
  join('features', 'voice', 'voiceService.ts'),
  join('features', 'voice', 'voice.test.ts'),
