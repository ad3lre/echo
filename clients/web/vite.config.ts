...
/**
 * Merge `VITE_*` from monorepo root and `clients/web/` so a single root `.env` works (and
 * `clients/web/.env` can still override). Default Vite `envDir` is only `clients/web/`, which ignored
