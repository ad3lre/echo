...
public final class EchoPushRegistration {
  ...
  private func loadBadgePreference() async {
    guard let baseURL, let accessToken else { return }
    if let prefs = try? await EchoSettingsClient(baseURL: baseURL)
      .loadNotificationPreferences(accessToken: accessToken)
    {
      EchoAppBadge.unreadBadgesEnabled = prefs.unreadBadge
      EchoForegroundNotifications.alertsEnabled = prefs.desktopAlerts
    }
  }
...
extension Notification.Name {
  ...
  public static let echoRemoteNotificationReceived = Notification.Name(
    "EchoRemoteNotificationReceived")
}

public enum EchoNotificationPayload {
  public static func channelID(from userInfo: [AnyHashable: Any]?) -> String? {
    if let value = userInfo?["channelId"] as? String {
