...
**Version alignment:** keep `version` in sync across root `package.json`, `src-tauri/Cargo.toml`, and `src-tauri/tauri.conf.json` (plus Android-specific fields in `src-tauri/tauri.android.conf.json` when shipping mobile). See platform docs for details.
