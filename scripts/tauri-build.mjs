...
// Skip the first `--` (npm passes it as a separator between the script name and
// forwarded args). Anything after it is forwarded verbatim to `tauri build`. Tauri
// itself uses its own `--` to split between its flags (e.g. `--no-bundle`) and
// the cargo passthrough (e.g. `-- --profile release-fast`).
