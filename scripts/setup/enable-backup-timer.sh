...
# Usage: sudo bash scripts/setup/enable-backup-timer.sh
set -euo pipefail
...
cp "$REPO_ROOT/infra/systemd/echo-backup.service" "$SERVICE"
cp "$REPO_ROOT/infra/systemd/echo-backup.timer" /etc/systemd/system/
...
if [ ! -s "$ENV_FILE" ]; then
  cp "$REPO_ROOT/infra/systemd/echo-backup.env.example" "$ENV_FILE"
fi
