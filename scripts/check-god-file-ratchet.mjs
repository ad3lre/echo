 ...
 *
 * - Each path in scripts/god-file-baselines.json has a frozen max line count (>= 700 at snapshot).
 ...
 * Intentional growth bypass: raise the path's line count in scripts/god-file-baselines.json
 */
...
function printViolations(result) {
  ...
  if (growthViolations.length) {
    ...
    console.error(
      '\nExtract logic into smaller modules, or intentionally raise the ceiling in scripts/god-file-baselines.json.',
    );
  }
  ...
  if (newGodViolations.length) {
    ...
    console.error(
      `\nAdd entries to scripts/god-file-baselines.json only when you deliberately accept a new oversized file.`,
    );
  }
  ...
  if (staleBaselines.length) {
    ...
    console.error(
      '\nRemove stale entries from scripts/god-file-baselines.json.',
    );
  }
