...
# Usage: sudo bash scripts/backup/install-backup.sh
...
if [ ! -f /etc/systemd/system/echo-backup.service ]; then
  cp infra/systemd/echo-backup.service /etc/systemd/system/
  cp infra/systemd/echo-backup.timer /etc/systemd/system/
