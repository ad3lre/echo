...
function createChatUploadTooLargeError(
  ...
  if (kind === 'document') {
    return new Error(
      `Document "${file.name}" is too large to upload (${mebibytes(ECHO_CLIENT_UPLOAD_MAX_BYTES)} MiB max).`,
    );
  }
  return new Error(
...
export function assertChatUploadFilesWithinClientLimits(opts: {
  ...
  pendingDocuments?: Array<{ file: FileLike }>;
}): void {
  ...
  for (const pending of opts.pendingDocuments ?? []) {
    if (pending.file.size > ECHO_CLIENT_UPLOAD_MAX_BYTES) {
      throw createChatUploadTooLargeError('document', pending.file);
    }
  }
}
