...
import { isServerChannelUnreadForPingBubble } from '@shared/attentionPing';
...
function buildDmAttentionByChannelId(
  ...
  return Object.fromEntries(
    Object.entries(channelAttentionByChannelId)
      .filter(([, summary]) => summary.kind === 'dm' && summary.unreadCount > 0)
...
export const useEchoAttentionStore = defineStore('echoAttention', () => {
  ...
  const dmAttentionByChannelId = computed(() =>
    buildDmAttentionByChannelId(channelAttentionByChannelId.value),
  );
