...
export const useServerAutomodRulesStore = defineStore(
  'serverAutomodRules',
  () => {
    ...
    async function createRule(
      ...
      try {
        ...
        setRules(
          serverId,
          [...prev, rule].sort((a, b) => a.position - b.position),
        );
        return rule;
      } catch (e) {
        lastError.value =
    ...
    async function saveRule(
      ...
      mutating.value = true;
      try {
        const { rule } = await updateEchoAutomodRule(
          token,
          serverId,
          ruleId,
          patch,
    ...
    async function reorder(
      ...
      mutating.value = true;
      try {
        const { rules } = await reorderEchoAutomodRules(
          token,
          serverId,
          orderedIds,
        );
