...
function handleSeenMessageIdChanged(messageId: string | null) {
  ...
  echoChannelHistory?.reportSeenMessageId(messageId);
}
...
watch(
  ...
  () => {
    ...
    echoChannelHistory?.reportSeenMessageId(null);
    debugForumNavLog({
      channelId: props.activeChannel?.id ?? null,
...
const imageList = computed<ImageItem[]>(() => {
  ...
  for (const m of msgs) {
    ...
    for (const att of atts) {
      if (!att?.url || att.kind === 'video') continue;
