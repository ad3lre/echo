        ...
        <div>
          ...
          <div class="relative">
            <select
              ...
              class="poll-time-limit-select w-full cursor-pointer appearance-none rounded-lg border border-border bg-surface py-2.5 pl-3 pr-10 text-sm text-foreground outline-none transition-[box-shadow,border-color] focus-visible:border-border focus-visible:ring-2 focus-visible:ring-accent/30"
            >
              <option
                v-for="opt in TIME_LIMIT_OPTIONS"
                :key="opt.label"
                :value="opt.value"
              >
                {{ opt.label }}
            ...
            <span
              class="pointer-events-none absolute inset-y-0 right-0 flex w-10 items-center justify-center text-muted"
              aria-hidden="true"
            >
              <svg
...
html[data-theme='light'] .poll-time-limit-select {
  color-scheme: light;
}

html[data-theme='dark'] .poll-time-limit-select {
  color-scheme: dark;
}
