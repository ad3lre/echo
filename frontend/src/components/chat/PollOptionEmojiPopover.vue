...
const categories = computed(() => getEmojiCategories());
const currentEmojiHtml = computed(() => {
  if (!props.modelValue) return '';
  const all = categories.value.flatMap((c) => c.emojis);
  const found = all.find((e) => e.emoji === props.modelValue);
  return found?.html ?? parseSingleEmoji(props.modelValue);
});
