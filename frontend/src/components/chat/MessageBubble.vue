...
function parseSingleEmojiForReactions(emoji: string): string {
  ...
  if (m && customEmojiUrlById?.value) {
    const url = customEmojiUrlById.value.get(m[2]!);
    if (url) {
...
function handleFocusOut(event: FocusEvent) {
  ...
  if (nextTarget && floatingActionBarWrapRef.value?.contains(nextTarget))
    return;
...
watch(isEditing, (editing) => {
  if (!editing) clearPendingEditMedia();
});
...
watch(() => message.value.id, resetForMessageChange);
</script>
      ...
      >
        ...
        <div data-dev-hit="message" class="min-w-0">
          ...
          <div
            ...
            ref="editFormRef"
            class="edit-form mt-1 rounded px-2 py-1 -mx-2 bg-scrim-1"
          >
            <textarea
              ref="editTextareaRef"
              v-model="editDraft"
              :rows="Math.min(20, Math.max(1, editDraft.split('\n').length))"
              placeholder="Edit message..."
              class="edit-textarea w-full resize-y min-h-[2rem] max-h-[50vh] overflow-auto border-none bg-transparent py-0.5 text-[0.9375rem] leading-[1.375rem] text-fg placeholder:text-muted outline-none"
            ...
            />
            <div
              class="flex flex-wrap items-center gap-2 border-t border-white/6 pt-2 mt-1"
            >
              <span
            ...
            <div class="flex items-center gap-3 pt-1 text-sm">
              <button
                type="button"
                class="chat-focus-ring px-3 py-2 rounded-md text-muted hover:text-foreground hover:bg-glass-hover transition-colors"
                @click="cancelEdit"
              >
                Cancel
              </button>
              <button
                type="button"
                class="chat-focus-ring px-3 py-2 rounded-md text-indigo-400 hover:text-indigo-300 hover:bg-glass-hover transition-colors"
                @click="() => void saveEdit()"
              >
                Save
              </button>
