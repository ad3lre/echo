...
<template>
  ...
  >
    ...
    >
      <div
        class="stream-video-tile__bar inline-flex min-w-0 flex-nowrap items-center gap-2 rounded-lg border border-border px-2 py-1.5 shadow-[0_4px_14px_rgba(0,0,0,0.35)]"
        :class="
          remoteStreamVolumeControl
            ? 'max-w-[min(100%,320px)]'
            : 'max-w-[min(100%,240px)]'
        "
      >
        ...
        <div
          ...
          class="stream-video-tile__stream-vol pointer-events-auto ml-0.5 flex min-w-0 max-w-[132px] shrink items-center gap-1 border-l border-border pl-2"
          @click.stop
          @pointerdown.stop
        >
          <input
            type="range"
            class="stream-video-tile__vol-range h-1 min-w-[52px] flex-1 cursor-pointer appearance-none rounded-full bg-glass-active accent-emerald-400"
            min="0"
            max="200"
            step="1"
    ...
    <div
      v-else-if="
        remoteStreamVolumeControl &&
        (hideParticipantBar || streamShareHideUserChrome)
      "
      class="pointer-events-none absolute bottom-3 left-3 z-20 max-w-[calc(100%-1.5rem)]"
    >
      <div
        class="stream-video-tile__bar stream-video-tile__stream-vol-floating pointer-events-auto inline-flex max-w-[min(100%,200px)] items-center gap-1.5 rounded-lg border border-border px-2 py-1 shadow-[0_4px_14px_rgba(0,0,0,0.35)]"
        @click.stop
        @pointerdown.stop
      >
        <input
          type="range"
          class="stream-video-tile__vol-range h-1 min-w-[56px] flex-1 cursor-pointer appearance-none rounded-full bg-glass-active accent-emerald-400"
          min="0"
          max="200"
          step="1"
          :value="streamVolumeSliderValue"
    ...
    <div
      class="pointer-events-none absolute right-3 top-3 z-30 flex gap-1.5 opacity-0 transition-opacity duration-200 group-hover:opacity-100 pointer-coarse:opacity-100"
    >
      <button
        v-if="pipSupported && track"
        type="button"
        class="pointer-events-auto flex h-9 w-9 items-center justify-center rounded-full bg-scrim-2 text-white shadow-lg backdrop-blur-sm transition-colors hover:bg-overlay-heavy"
