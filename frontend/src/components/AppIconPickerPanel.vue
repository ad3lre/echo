...
} from '@/composables/useAppIconSearch';
...
<template>
  ...
  >
    ...
    >
      Icons are grouped by theme (chat, voice, people) and by variant — click a
      tile with a badge to expand.
    </p>
    <p
      v-else-if="showHints && !isBrowsing"
      class="px-3 pb-1 text-[10px] leading-snug text-fg-subtle"
    >
      Search uses the same groups — chat, voice, and people icons collapse into
      one tile each when they match.
    </p>
