...
const sortedServers = computed(() => {
  ...
  if (mode === 'members') {
    return list.sort(
      (a, b) => b.memberCount - a.memberCount || a.order - b.order,
    );
  }
  ...
  if (mode === 'trendy') {
    return list.sort(
      (a, b) =>
        trendyScore(b, listLen) - trendyScore(a, listLen) || a.order - b.order,
    );
  }

  return list.sort(
    (a, b) =>
      recommendedScore(b, listLen) - recommendedScore(a, listLen) ||
      a.order - b.order,
  );
});
