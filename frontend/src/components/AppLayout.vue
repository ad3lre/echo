...
const appToastShellClass = computed(() => {
  const y = chatComposerFocusedForToast.value
    ? 'bottom-[max(8rem,calc(env(safe-area-inset-bottom,0px)+6rem))]'
    : 'bottom-[max(1rem,env(safe-area-inset-bottom,0px))]';
  const shell =
    'box-border flex min-h-0 max-h-[min(90dvh,calc(100dvh-2rem))] flex-col overflow-hidden overscroll-contain';
          ...
          <div class="app-toast-incoming-call__inner flex items-start gap-2.5">
            ...
            <div class="min-w-0 flex-1">
              ...
              >
                ...
                <p
                  class="mt-2 line-clamp-3 text-sm leading-snug text-fg-soft [overflow-wrap:anywhere] max-sm:line-clamp-none max-sm:max-h-[min(12rem,42vh)] max-sm:overflow-y-auto max-sm:overscroll-y-contain"
                >
                  {{ appToast.message }}
                </p>
                <div
