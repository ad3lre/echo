...
function commitCustomStatus() {
  emit('update:customStatus', draftStatus.value.trim());
          ...
          <div class="self-popout__status-row">
            ...
            <div class="min-w-0 flex-1 self-popout__status-value">
              <input
                ...
                @keydown.enter.prevent="commitCustomStatus"
                @keydown.esc.prevent="isEditingCustomStatus = false"
                @click.stop
              />
...
/* Reserve at least one line so swapping <p> ↔ input does not jump the popout. */
...
.self-popout__status-input {
  display: block;
  /* Match single-line <p> line box; tame UA input chrome so height matches view mode. */
  height: 1.45rem;
  min-height: 1.45rem;
