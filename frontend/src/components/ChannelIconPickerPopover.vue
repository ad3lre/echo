...
const triggerDisplayUrl = ref(resolveIconKeyToUrl(props.modelValue));
const triggerEmoji = computed(() => parseEmojiIconKey(props.modelValue));
const triggerIsEmoji = computed(() => !!triggerEmoji.value);

async function syncTriggerIconUrl() {
  if (parseEmojiIconKey(props.modelValue)) return;
  if (isEchoChannelIconImageUrlKey(props.modelValue)) {
    triggerDisplayUrl.value = safeImageUrl(props.modelValue);
    return;
  }
  await ensureIconCatalogLoaded();
  triggerDisplayUrl.value = resolveIconKeyToUrl(props.modelValue);
}
...
function open() {
  ...
  emojiSearchQuery.value = '';
  activeTab.value = parseEmojiIconKey(props.modelValue) ? 'emoji' : 'icons';
...
function handleEmojiPick(entry: EmojiEntry) {
  if (entry.kind === 'custom') {
    ...
    if (sid && entry.serverId === sid && entry.id) {
  ...
  >
    ...
    >
      ...
      <img
        ...
        alt=""
        class="object-contain opacity-90 filter invert"
        :class="variant === 'combined' ? 'h-5 w-5' : 'h-6 w-6'"
      />
    </button>
    <Teleport to="body">
      <div
