    ...
    <div class="grid gap-4 lg:grid-cols-2">
      ...
      <div class="settings-card rounded-2xl p-5">
        ...
        <div class="mt-4 flex flex-col gap-3">
          ...
          <select
            :value="selectedId"
            class="settings-input max-w-none"
            @change="
              ringtoneStore.setRingtoneById(
                String(($event.target as HTMLSelectElement).value),
              )
            "
          >
            <template v-for="group in ringtoneOptionGroups" :key="group.label">
              <optgroup :label="group.label">
                <option
                  v-for="entry in group.entries"
                  :key="entry.id"
                  :value="entry.id"
                  class="bg-[var(--echo-settings-inset-bg)] text-foreground"
                >
                  {{ entry.label }}
