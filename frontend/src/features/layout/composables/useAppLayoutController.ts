...
import { getChannelDisplayName, getChannelIcon, icons } from '@/assets/icons';
...
import { buildEchoServerMarkReadPlan } from '@/services/domain/echoServerMarkReadTargets';
...
export function useAppLayoutController() {
  ...
  const {
    ...
    markServerAsRead: async (serverId: string) => {
      ...
      if (!plan.targets.length && !plan.channelIdsMissingLatestUnread.length) {
        dispatchAppToast('No unread channels in this server.', 'info');
        return;
      }
      // Apply local cursor patches first so rail badges clear immediately.
      for (const t of plan.targets) {
        echoAttention.patchReadState(t.channelId, t.lastReadMessageId);
      }
      ...
      for (const t of plan.targets) {
        try {
          ...
          failed += 1;
          reportPrimaryFlowFailure('server_mark_read_failed', e, undefined, {
            showBanner: false,
          });
        }
      }
      if (plan.channelIdsMissingLatestUnread.length > 0) {
        failed += plan.channelIdsMissingLatestUnread.length;
      }
      if (failed > 0) {
  ...
  async function markEchoChannelAsRead(
    ...
    echoAttention.patchReadState(cid, targetId);
    const token = authSession.accessToken?.trim() ?? '';
    try {
  ...
  const profileSurfaceAdapter = useProfileSurfaceAdapter({
    ...
    onUpdateExpandedProfileNote: (note: string) => {
      const id = expandedProfile.value?.id ?? null;
