...
import { getChannelIcon, getChannelIconVisual, icons } from '@/assets/icons';
...
const voiceSideChatIconVisual = computed(() =>
  getChannelIconVisual(props.effectiveActiveChannel ?? null),
);
...
const voiceSideChatIconUrl = computed(() => {
  ...
  return v.kind === 'svg'
    ? v.url
    : getChannelIcon(props.effectiveActiveChannel ?? null);
});
  ...
  <template v-if="isViewingVoiceChannel">
    ...
    >
      ...
      >
        ...
        <!-- Mobile: bottom chat sheet — hidden until scroll-up on call; half then full height. -->
        <div
          v-else-if="!voiceSideChatCollapsed"
