...
<template>
  <div
    ...
    :class="
      props.horizontal
        ? 'relative flex min-h-0 w-full max-w-full flex-row items-center justify-center overflow-hidden'
  ...
  >
    <div
      ...
      :class="
        props.horizontal
          ? 'flex min-h-0 w-auto max-w-full min-w-0 flex-row flex-nowrap items-center justify-center gap-3 overflow-x-auto overflow-y-hidden px-3 py-2 sm:gap-4 md:px-6'
    ...
    >
      <template v-if="authenticated && props.horizontal">
        ...
        <div
          class="flex min-h-0 min-w-0 max-w-[min(760px,58vw)] flex-row items-center gap-2 overflow-x-auto overflow-y-hidden [scrollbar-width:thin]"
        >
          <button
            type="button"
            class="explore-trigger relative flex h-10 w-10 shrink-0 cursor-pointer items-center justify-center border-0 bg-transparent p-0 text-inherit"
