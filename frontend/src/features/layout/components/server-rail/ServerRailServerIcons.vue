...
<template>
  <div
    ...
    :class="[
      horizontal
        ? 'servers-folder--horizontal mt-0 max-h-12 min-h-0 w-auto min-w-0 flex-row flex-nowrap items-center overflow-x-auto overflow-y-hidden opacity-100'
        : 'w-full flex-col',
  ...
  >
    ...
    >
      <span
        ...
        :class="[
          horizontal
            ? 'left-1/2 top-0 -translate-x-1/2 -translate-y-1/2 server-folder__unread-activity-dot--horizontal'
      ...
      >
        ...
        <span
          v-if="
            ...
            !serverPingChannelDots?.[server.id]?.dots?.length
          "
          class="pointer-events-none absolute -left-0.5 -top-0.5 z-[1] flex h-[18px] min-w-[18px] items-center justify-center rounded-full border-2 border-[var(--bg)] px-0.5 text-[10px] font-bold tabular-nums leading-none text-white shadow-sm"
...
/* compact white sliver where the selection pill sits — plain unreads only */
...
.server-folder__slot--unread-activity:hover
  .server-folder__unread-activity-dot {
  opacity: 0;
}
...
.server-folder__overflow-pill {
  ...
  background: color-mix(in srgb, var(--vue-auto-012) 22%, transparent);
  color: color-mix(in srgb, white 92%, var(--fg) 8%);
  box-shadow:
...
.server-folder__overflow-pill:hover {
  background: color-mix(in srgb, var(--vue-auto-012) 30%, transparent);
}
