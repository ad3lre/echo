...
const compactGuildMembersPaneFocused = computed(() => {
  const v = layoutMembers?.compactGuildMembersPaneFocused;
  if (v === undefined) return false;
  return !!unref(v);
});

const memberPanelCollapsedRaw = computed(
  () => pick('memberPanelCollapsed').value ?? false,
);

/** Members surface is open: desktop expanded list, or compact tri-pane on the members pager. */
const membersPanelOpenForSearchChrome = computed(
  () => compactGuildMembersPaneFocused.value || !memberPanelCollapsedRaw.value,
);

/** Search is active in this column (query or filters) — same panel as header search when list is collapsed. */
const memberColumnSearchActive = computed(() => {
  const q = (resolvedSearchText.value ?? '').trim();
  if (q.length > 0) return true;
  return (resolvedFilterChips.value?.length ?? 0) > 0;
});

const { isCompactShell } = useCompactShell();

/** Collapse control is for desktop split view; on compact shell use sheet navigation instead of a back-style arrow beside search. */
const showCollapseMembersWhileSearching = computed(
  () =>
    membersPanelOpenForSearchChrome.value &&
    memberColumnSearchActive.value &&
    !isCompactShell.value,
);

function collapseMembersFromSearch() {
...
<template>
  ...
  >
    <div
      ...
      class="chat-header-glass pointer-events-auto absolute top-0 left-0 right-0 z-20 flex h-12 flex-shrink-0 items-center justify-end"
    >
      <div
        class="search-input-wrapper relative flex h-full min-w-0 w-full flex-shrink flex-row items-end gap-1"
      >
        <div class="relative flex min-w-0 flex-1 flex-col items-end">
          <SearchBar
            ...
            "
            :reserved-right-px="showCollapseMembersWhileSearching ? 36 : 0"
            :dropdown-gap-px="0"
        ...
        <button
          v-if="showCollapseMembersWhileSearching"
          type="button"
          class="dm-header-action-btn mb-0.5 shrink-0"
          title="Hide member list"
          aria-label="Hide member list"
          @click="collapseMembersFromSearch"
        >
          <img
            :src="icons.arrowLeft"
            alt=""
            class="dm-header-action-icon rotate-180"
          />
        </button>
      </div>
    </div>
