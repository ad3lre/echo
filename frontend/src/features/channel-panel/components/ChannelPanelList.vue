...
function getChannelEmojiOrNull(channel: {
  ...
  const v = getChannelIconVisual(channel);
...
function getChannelIconUrlOrFallback(channel: {
  ...
  const v = getChannelIconVisual(channel);
  return v.kind === 'svg' ? v.url : getChannelIcon(channel);
}
        ...
        <div class="flex flex-col items-center gap-1">
          ...
          >
            <img
              ...
              :class="{ 'opacity-60': channel.id !== activeChannelId }"
            />
            <span
            ...
            <div
              ...
              class="absolute -bottom-0.5 -right-0.5 h-2.5 w-2.5 rounded-full bg-indigo-500"
            />
            ...
            <div
              ...
              "
              class="absolute -top-0.5 -right-0.5 h-2.5 w-2.5 rounded-full bg-emerald-500"
            />
          </button>
        </div>
      </div>
          ...
          >
            ...
            >
              <button
                type="button"
                draggable="false"
                class="channel-category-gear flex h-5 w-5 items-center justify-center rounded text-fg-subtle opacity-0 transition-all hover:bg-glass-hover hover:text-fg group-hover:opacity-100 pointer-coarse:opacity-100"
                title="Category settings"
                aria-label="Category settings"
                @click.stop="emit('open-category-settings', category.id)"
              >
                <img
                  :src="icons.settings"
                  alt=""
        ...
        >
          ...
          >
            ...
            >
              ...
              >
                <div
                  class="channel-row-drop-target flex items-center gap-2 min-w-0 rounded-md transition-colors"
                  :class="
                    channel.type === 'voice'
                ...
                >
                  <div class="flex min-w-0 flex-1 items-center gap-2">
                    <img
                      ...
                      alt=""
                      draggable="false"
                      class="channel-row-icon h-4 w-4 flex-shrink-0 filter invert"
                      :class="{
                  ...
                  <button
                    ...
                    "
                    type="button"
                    class="channel-row-gear ml-auto flex h-7 w-7 shrink-0 items-center justify-center rounded-md text-fg-subtle opacity-0 transition-opacity hover:bg-glass-hover hover:text-foreground group-hover:opacity-100 pointer-coarse:opacity-100"
                  ...
                  >
                    <button
                      ...
                      "
                      type="button"
                      class="vc-row-btn opacity-0 transition-opacity group-hover:opacity-100 pointer-coarse:opacity-100"
                      title="Channel settings"
                      aria-label="Channel settings"
                      @click="
                        emit('open-channel-settings', {
              ...
              <div class="mt-1 flex flex-col gap-1">
                <div
                  ...
                  :class="{
                    ...
                    'channel-row--has-unread': channelRowMissedActivity(sub.id),
                  }"
                  title="Forum post"
          ...
          >
            <div ref="quickCreateStackRef" class="channel-quick-create__stack">
              ...
              <template v-else>
                ...
                >
                  <label
                    class="channel-quick-create__label"
                    :for="`quick-create-category-${selectedServerId ?? 'server'}`"
                  >
                    Category
                  </label>
                  <select
                    :id="`quick-create-category-${selectedServerId ?? 'server'}`"
                    v-model="quickCreateCategoryId"
                    class="channel-quick-create__select"
                    :disabled="!quickCreateCanUseChannels"
                  >
                    <option
...
.channel-quick-create__select:focus {
  box-shadow: 0 0 0 2px color-mix(in srgb, var(--accent) 22%, transparent);
}

.channel-quick-create__select:disabled {
  cursor: not-allowed;
  opacity: 0.65;
}
...
/** Guild sidebar: unread vs read — `channelMissedActivityByChannelId` + `.channel-row--has-unread` */
.channel-row.channel-row--has-unread {
  position: relative;
}

.channel-row.channel-row--has-unread::before {
  content: '';
  position: absolute;
  left: 0.12rem;
  top: 50%;
...
.forum-post-sidebar-row.channel-row--has-unread::before {
  left: 0.08rem;
  height: 1rem;
}
