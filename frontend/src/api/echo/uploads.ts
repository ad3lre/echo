...
async function uploadPreparedFileWithDedupe(
  ...
  const { sha256Hex, phashHex } =
    kind === 'video' && options?.videoDedupeFastPath
      ? {
...
/**
 * Presign + PUT for chat attachments (images/videos via dedupe pipeline; audio direct PUT).
...
export async function uploadChatAttachmentFile(
  ...
  const kind: 'image' | 'video' | 'audio' = isVideo
    ? 'video'
    : isAudio
      ? 'audio'
      : 'image';
  ...
  const [presign, audioSha256Hex] = await Promise.all([
  ...
  emitIf('done', null);
  return { url: presign.publicUrl, kind };
}
