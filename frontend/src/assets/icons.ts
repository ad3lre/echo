...
/**
 * True when `iconKey` is persisted as an image URL/path (e.g. custom emoji asset),
...
export function isEchoChannelIconImageUrlKey(
  ...
  if (!isTrustedMediaUrl(t)) return false;
...
export function resolveIconKeyToUrl(key: string): string {
  ...
  if (key && isEchoChannelIconImageUrlKey(key)) return safeImageUrl(key);
...
export function getChannelIconVisual(
  ...
  if (key) {
    ...
    if (isEchoChannelIconImageUrlKey(key)) {
      return { kind: 'svg', url: safeImageUrl(key) };
    }
  }
  if (!key) {
...
export function getChannelIcon(
  ...
  if (key) {
    ...
    if (isEchoChannelIconImageUrlKey(key)) return safeImageUrl(key);
  }
  if (!key) {
