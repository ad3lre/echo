...
export default async function echoUploadsRoutes(
  ...
  fastify.post<{ Body: EchoPresignBody }>(
    '/uploads/presign',
    { preHandler: [requireAuth, requireEchoStore], bodyLimit: 32 * 1024 },
    async (req, reply) => {
  ...
  fastify.post<{ Body: EchoDedupeMatchBody }>(
    ...
    { preHandler: [requireAuth, requireEchoStore], bodyLimit: 4096 },
    async (req, reply) => {
      const pool = echoPool(req);
      const userId = req.authUser!.id;
      const body = req.body;
  ...
  fastify.post<{ Body: EchoDedupeRegisterBody }>(
    ...
    { preHandler: [requireAuth, requireEchoStore], bodyLimit: 8192 },
    async (req, reply) => {
      const pool = echoPool(req);
      const userId = req.authUser!.id;
      const body = req.body;
