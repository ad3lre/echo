...
export default async function googleOAuthRoutes(
  ...
  fastify.get<{
    ...
    async (req: FastifyRequest, reply: FastifyReply) => {
      ...
      if (!fetchSite) {
        ...
        const appBase = config.echoAppPublicUrl.trim().replace(/\/$/, '');
        const allowed =
          (origin && origin.startsWith(appBase)) ||
          (referer && referer.startsWith(appBase));
        if (!allowed) {
          return sendError(
            reply,
            403,
  ...
  fastify.get<{
    ...
    let me: Awaited<ReturnType<typeof fetchGoogleUserInfo>>;
    try {
      me = await fetchGoogleUserInfo(tokenResponse.access_token, OAUTH_FETCH);
    } catch (e) {
