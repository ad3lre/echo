...
export async function getEchoChannelAuditSnapshot(
  ...
  const r = await pool.query(
    `SELECT permission_overrides, name, category_id, slowmode_seconds, user_limit, bitrate_bps, voice_e2ee_enabled, nsfw, icon_key, message_history_anchor FROM echo_channels WHERE id = $1 AND server_id = $2 LIMIT 1`,
    [channelId, serverId],
  );
