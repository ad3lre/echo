...
> {
  const r = await pool.query(
    `
    SELECT s.id, s.name, s.icon_url, s.banner_url, s.banner_position_y, s.owner_id, s.listed_in_directory, s.banner_blur_enabled, s.banner_blackout_enabled,
           s.automod_spam_enabled, s.raid_protection_enabled, s.raid_join_threshold_count, s.raid_join_window_seconds,
           s.vanity_code, s.description, s.tags, ist.discord_guild_id
    FROM echo_servers s
...
export type JoinEchoInviteResult =
  ...
  | {
      ok: false;
      reason: 'not_found' | 'banned' | 'server_limit' | 'raid_protection';
    };
