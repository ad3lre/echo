...
> {
  const r = await pool.query(
    ...
    SELECT ch.id, ch.name, ch.type, ch.position, ch.permission_overrides,
           ...
           ch.message_history_anchor, ch.discord_voice_mirror_only,
...
export async function listEchoCategories(
  ...
  const r = await pool.query(
    `SELECT id, name, position FROM echo_categories WHERE server_id = $1 ORDER BY position ASC, name ASC`,
    [serverId],
  );
...
export function mergeCategoriesAndUncategorizedRoots(
  serverId: string,
  cats: { id: string; name: string; position: number }[],
  ...
  type Slot =
    | {
        ...
        pos: number;
        tie: string;
        cat: { id: string; name: string; position: number };
      }
...
export async function listEchoWorkspaceForUser(
  ...
  return { servers, categoriesByServer, membersByServer, workspaceVersion };
}
