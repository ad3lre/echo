...
import { refreshYoutubeOAuthToken } from '../services/integrations/youtubeApiClient';
import {
  getYoutubeLinkByUserId,
  updateYoutubeChannelLinkTokens,
} from './youtubeUserLinkRepo';
...
/** Returns a usable YouTube OAuth access token for the linked channel, refreshing when needed. */
export async function getYoutubeUserAccessTokenForApi(
  pool: Pool,
  userId: string,
): Promise<string> {
  const row = await getYoutubeLinkByUserId(pool, userId);
  ...
  const next = await refreshYoutubeOAuthToken(refreshPlain);
  ...
  await updateYoutubeChannelLinkTokens(pool, userId, {
