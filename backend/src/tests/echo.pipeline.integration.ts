...
async function run(): Promise<void> {
  ...
  try {
    ...
    assert.deepEqual(categoryOrder, ['Zebra', 'AAA', 'Text Channels']);
    ...
    const guestRes = await fetch(`${baseUrl}/api/v1/auth/continue-as-guest`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ displayName: 'Guest DM Test' }),
    });
    ...
    // Guest joins server srv (where t1 is owner)
    ...
    assert.ok(
      friendReqToGuestBody.toLowerCase().includes('guest'),
      friendReqToGuestBody,
    );

    // Guest can now DM t1 (shared server)
    const guestDmShared = await fetch(`${baseUrl}/api/v1/echo/dm/open`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'x-csrf-token': guest.csrfToken,
      ...
      body: JSON.stringify({ peerUserId: t1UserId }),
    });
    assert.equal(
      guestDmShared.status,
      200,
      'Guest should be able to DM member of shared server',
    );
