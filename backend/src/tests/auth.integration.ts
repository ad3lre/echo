...
async function run(): Promise<void> {
  ...
  const unsupportedEmailRes = await app.inject({
    method: 'POST',
    url: '/api/v1/auth/register',
    payload: {
      username: `corp_${Date.now().toString(36)}`,
      password: 'password123',
      email: 'person@unknown-corp.invalid',
    },
  });
  assert.equal(unsupportedEmailRes.statusCode, 400, unsupportedEmailRes.body);
  assert.equal(
    (unsupportedEmailRes.json() as { code?: string }).code,
