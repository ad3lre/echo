...
function resolveEchoLocalUploadDir(): string | null {
  ...
  return path.resolve(
    raw || path.join(process.cwd(), 'data', 'echo-local-uploads'),
  );
}
...
if (config.isProduction) {
  ...
  if (
    ...
    (!isStrongProductionSecret(config.localUploadTokenSecret) ||
      config.localUploadTokenSecret === config.jwtSecret)
  ) {
    configStderr(
      `LOCAL_UPLOAD_TOKEN_SECRET is required in production when local uploads are enabled. Use at least ${MIN_PRODUCTION_SECRET_LENGTH} random characters independent of JWT_SECRET.`,
    );
    process.exit(1);
  }
  if (config.echoDiscordBotWebhookSecret === DEV_DISCORD_BOT_WEBHOOK_SECRET) {
    configStderr(
  ...
  if (
    ...
    !isStrongProductionSecret(config.echoDiscordBotWebhookSecret)
  ) {
    configStderr(
  ...
  const corsOriginRaw = normalizeEnvValue(process.env.CORS_ORIGIN);
  ...
  if (
    ...
    !isStrongProductionSecret(config.echoMetricsScrapeToken)
  ) {
    configStderr(
      `ECHO_METRICS_SCRAPE_TOKEN is required in production so GET /api/v1/metrics is not world-readable. Set at least ${MIN_PRODUCTION_SECRET_LENGTH} random characters and scrape with Authorization: Bearer <token>, block the route at your reverse proxy, or set ECHO_REQUIRE_METRICS_SCRAPE_TOKEN_IN_PRODUCTION=false only for deliberate single-process / local smoke stacks.`,
    );
    process.exit(1);
  }
  if (
    config.echoAgentNetworkDiagnosticsEnabled &&
    !isStrongProductionSecret(config.echoAgentNetworkDiagnosticsToken)
  ) {
    configStderr(
      `ECHO_AGENT_NETWORK_DIAG_ENABLED is on in production but ECHO_AGENT_NETWORK_DIAG_TOKEN is missing or too weak. Set at least ${MIN_PRODUCTION_SECRET_LENGTH} random characters (Authorization: Bearer <token>) or disable the feature.`,
    );
    process.exit(1);
  }
  if (
